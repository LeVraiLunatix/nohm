            ...
            <div className="music-player">
              ...
              <div className="music-player-main">
                ...
                <div className="music-controls">
                  <MusicButton label={t('music.shuffle')} disabled={!data.capabilities.shuffle || pending} onClick={() => void command('toggle-shuffle')}>⌘</MusicButton>
                  <MusicButton label={t('music.previous')} disabled={!data.capabilities.previous || pending} onClick={() => void command('previous')}>‹</MusicButton>
                  <MusicButton label={data.playing ? t('music.pause') : t('music.play')} disabled={pending} onClick={() => void command(data.playing ? 'pause' : 'play')}>{data.playing ? 'Ⅱ' : '▶'}</MusicButton>
                  <MusicButton label={t('music.next')} disabled={!data.capabilities.next || pending} onClick={() => void command('next')}>›</MusicButton>
                  <MusicButton label={t('music.repeat')} disabled={!data.capabilities.repeat || pending} onClick={() => void command('toggle-repeat')}>↻</MusicButton>
                </div>
