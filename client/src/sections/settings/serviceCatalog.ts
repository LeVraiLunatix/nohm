...
export interface ServiceDefinition {
  ...
  localOnly?: boolean;
  fields?: ServiceField[];
  oauth?: 'gmail' | 'spotify' | 'github' | 'steam' | 'lastfm';
}
...
export const SERVICE_CATALOG: ServiceDefinition[] = [
  ...
  { id: 'cider', name: 'Cider', widgetIds: ['music-cider'], permissions: { fr: 'Contrôle du lecteur local via RPC officiel', en: 'Control local player through official RPC' }, setup: { fr: 'Jeton RPC du lecteur local Cider', en: 'RPC token of the local Cider player' }, refresh: '5 s', localOnly: true, fields: [secret('CIDER_RPC_TOKEN', 'Jeton RPC', 'RPC token'), { ...text('CIDER_RPC_URL', 'Adresse RPC (si port modifié)', 'RPC address (if port changed)', 'http://127.0.0.1:10767'), type: 'url', optional: true }, { key: 'CIDER_RPC_UNAUTHENTICATED', label: { fr: 'Cider sans jeton (auth désactivée)', en: 'Cider without a token (auth disabled)' }, type: 'checkbox', optional: true }] },
  ...
  { id: 'lastfm', name: 'Last.fm', widgetIds: ['music-lastfm'], permissions: { fr: 'Historique, favoris et statistiques en lecture seule', en: 'Read-only history, favorites and statistics' }, setup: { fr: 'Se connecter avec Last.fm + clé d’application', en: 'Sign in with Last.fm + app key' }, refresh: '1 min', oauth: 'lastfm', fields: [secret('LASTFM_API_KEY', 'Clé API', 'API key'), secret('LASTFM_SECRET', 'Secret API', 'API secret'), { ...text('LASTFM_USER', 'Nom d’utilisateur (rempli par « Se connecter »)', 'Username (filled by "Sign in")'), optional: true }] },
