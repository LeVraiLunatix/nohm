...
export const SERVICE_CATALOG: ServiceDefinition[] = [
  ...
  { id: 'github', name: 'GitHub', widgetIds: ['github'], permissions: { fr: 'Dépôts et activité ; écriture séparée pour les issues', en: 'Repositories and activity; separate issue write permission' }, setup: { fr: 'Jeton conservé uniquement par le serveur local', en: 'Token kept only by the local server' }, refresh: '5 min', fields: [text('GITHUB_USERNAME', 'Nom d’utilisateur', 'Username'), secret('GITHUB_TOKEN', 'Jeton en lecture', 'Read token'), secret('GITHUB_ISSUES_TOKEN', 'Jeton de création d’issues (facultatif)', 'Issue creation token (optional)', true)] },
  ...
  { id: 'steam', name: 'Steam', widgetIds: ['steam'], permissions: { fr: 'Profil, bibliothèque et succès en lecture seule', en: 'Read-only profile, library and achievements' }, setup: { fr: 'Clé Web API et SteamID64', en: 'Web API key and SteamID64' }, refresh: '15 min', fields: [text('STEAM_ID', 'SteamID64', 'SteamID64', '7656119…'), secret('STEAM_API_KEY', 'Clé Web API Steam', 'Steam Web API key')] },
