...
export const serviceSettingsSchemas = {
  ...
  // Connect is PKCE, so only the client id is required; a secret is accepted but optional.
  spotify: z.object({ SPOTIFY_CLIENT_ID: z.string().trim().min(1), SPOTIFY_CLIENT_SECRET: z.string().optional() }),
